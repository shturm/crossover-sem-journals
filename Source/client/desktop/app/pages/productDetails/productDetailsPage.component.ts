...
export class ProductDetailsPageComponent implements OnInit {
    ...
    subscribe(journal: Product) {
        this.productsService.subscribeToJournal(journal).then(() => {
            this.router.navigate(['browse']);
        });
    }

    unsubscribe(journal: Product) {
        this.productsService.unsubscribeFromJournal(journal).then(() => {
            this.router.navigate(['browse']);
        });
    }
