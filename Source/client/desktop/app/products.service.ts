...
export class ProductsService {
    ...
    findProduct(journalId: string): Observable<any> {
        let headers = new Headers();
        this.auth.authorizeHeaders(headers);
        let opts: RequestOptionsArgs = { headers: headers };

        let result = new Observable<Product>((sub: Subscriber<any>) => {
            this.http.get('http://localhost:8080/api/journals/subscription?journalId=' + encodeURIComponent(journalId), opts)
                .subscribe(journalResponse => {
                    this.http.get('http://localhost:8080/api/papers?journalId=' + encodeURIComponent(journalId), opts)
    ...
    getProducts(term: string): Observable<Product[]> {
        let result = new Observable<Product[]>((sub: Subscriber<Product[]>) => {
            ...
            this.auth.authorizeHeaders(headers);
            let opts: RequestOptionsArgs = { headers: headers };
            let url = 'http://localhost:8080/api/journals/subscriptions';
            if (term) {
    ...
    subscribeToJournal(journal: Product): Promise<any> {
        let headers = new Headers();
        this.auth.authorizeHeaders(headers);
        let opts: RequestOptionsArgs = { headers: headers };
    ...
    unsubscribeFromJournal(journal: Product): Promise<any> {
        let headers = new Headers();
        this.auth.authorizeHeaders(headers);
        let opts: RequestOptionsArgs = { headers: headers };
    ...
    addPaperToJournal(paper: Paper, journalId: number): Promise<any> {
        let pr = new Promise((resolve, reject) => {
            ...
            xhr.setRequestHeader("Authorization", this.auth.getAuthorizationHeaderValue());
